...
struct sstp_state
{
    ...
    /*! Argument for the forward callaback */
    void *fwctx;
    ...
    uint8_t mppe_send_key[SSTP_MPPE_MAX_KEYLEN];

    /*! The MPEE receive key for HLAK */
    uint8_t mppe_recv_key[SSTP_MPPE_MAX_KEYLEN];

    /*! The actual size of the MPPE key received */
    size_t mppe_key_len;
};
...
static status_t sstp_state_send_connect(sstp_state_st *ctx)
{
    ...
    sstp_cmac_send_key(cmac, ctx->mppe_send_key, ctx->mppe_key_len);
    sstp_cmac_recv_key(cmac, ctx->mppe_recv_key, ctx->mppe_key_len);
